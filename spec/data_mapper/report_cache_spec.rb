...
describe ReportCache do

  before do
    @report = create_report(User, :registrations, :limit => 10)
  end

  describe 'validations' do

    before do
      @report_cache = create_report_cache(
  ...
  describe '.clear_for' do
    ...
    it 'should delete all entries in the cache for the klass and report name' do
      ReportCache.should_receive(:delete_all).once.with(:conditions => {
        :model_name  => User.name,
        :report_name => 'registrations'
      })

      ReportCache.clear_for(User, :registrations)
    end
  ...
  describe '.process' do
    ...
    before do
      ReportCache.stub!(:find).and_return([])
      ReportCache.stub!(:prepare_result).and_return([])
    end

    it 'should raise an ArgumentError if no block is given' do
      lambda do
        ReportCache.process(@report, @report.options)
      end.should raise_error(ArgumentError)
    end

    it 'sould start a transaction' do
      ReportCache.should_receive(:transaction)
    ...
    describe 'with :live_data = true' do
      ...
      it 'should yield to the given block' do
        lambda {
          ReportCache.process(@report, @options) { raise YieldMatchException.new }
      ...
      it 'should yield the first reporting period if not all required data could be retrieved from the cache' do
        reporting_period = create_reporting_period(
          @report.options[:grouping],
          Time.now - 3.send(@report.options[:grouping].identifier)
        )
        ReportCache.stub!(:all).and_return([ReportCache.new])

        ReportCache.process(@report, @options) do |begin_at, end_at|
          begin_at.should == ReportingPeriod.first(@report.options[:grouping], @report.options[:limit]).date_time
          end_at.should   == nil
          []
        end
      end
      ...
      it 'should yield the reporting period after the last one in the cache if all required data could be retrieved from the cache' do
        ...
        cached = ReportCache.new
        cached.stub!(:reporting_period).and_return(reporting_period.date_time)
        ReportCache.stub!(:all).and_return(Array.new(@report.options[:limit] - 1, ReportCache.new), cached)
    ...
    describe 'with :live_data = false' do
      ...
      it 'should not yield if all required data could be retrieved from the cache' do
        ReportCache.stub!(:all).and_return(Array.new(@report.options[:limit], ReportCache.new))

        lambda {
          ReportCache.process(@report, @report.options) { raise YieldMatchException.new }
        }.should_not raise_error(YieldMatchException)
      end

      it 'should yield to the block if no data could be retrieved from the cache' do
        ReportCache.stub!(:all).and_return([])
      ...
      describe 'with :end_date = <some date>' do
        ...
        it 'should yield the last date and time of the reporting period for the specified end date' do
          reporting_period = create_reporting_period(@report.options[:grouping], @options[:end_date])
    ...
    it 'should read existing data from the cache' do
      ReportCache.should_receive(:all).once.with(
        :conditions => [
          %w(model_name report_name grouping aggregation conditions).map do |column_name|
            "#{ReportCache.connection.quote_column_name(column_name)} = ?"
          ...
          '',
          ReportingPeriod.first(@report.options[:grouping], 10).date_time
        ],
        :limit => 10,
    ...
    it 'should utilize the end_date in the conditions' do
      ...
      ReportCache.should_receive(:all).once.with(
        :conditions => [
          ...
          @report.aggregation.to_s,
          '',
          ReportingPeriod.first(@report.options[:grouping], 9).date_time,
          create_reporting_period(@report.options[:grouping], end_date).date_time
        ],
        :limit => 10,
        :order => 'reporting_period ASC'
      ).and_return([])

      ReportCache.process(@report, @report.options.merge(:end_date => end_date)) { [] }
    end
    ...
    it "should read existing data from the cache for the correct grouping if one other than the report's default grouping is specified" do
      ...
      ReportCache.should_receive(:all).once.with(
        :conditions => [
          ...
          @report.aggregation.to_s,
          '',
          ReportingPeriod.first(grouping, 10).date_time
        ],
        :limit => 10,
        :order => 'reporting_period ASC'
      ).and_return([])

      ReportCache.process(@report, { :limit => 10, :grouping => grouping }) { [] }
    end

    it 'should yield the first reporting period if the cache is empty' do
      ReportCache.process(@report, @report.options) do |begin_at, end_at|
        begin_at.should == ReportingPeriod.first(@report.options[:grouping], 10).date_time
        end_at.should == nil
        []
      end
    end
  ...
  describe '.serialize_conditions' do
    ...
    it 'should serialize empty conditions correctly' do
      result = ReportCache.send(:serialize_conditions, [])
      result.should eql('')
    end
    
    it 'should serialize a conditions array correctly' do
      result = ReportCache.send(:serialize_conditions, ['active = ? AND gender = ?', true, 'male'])
      result.should eql('active = ? AND gender = ?truemale')
    end
    
    it 'should serialize a conditions hash correctly' do
      result = ReportCache.send(:serialize_conditions, { :gender => 'male', :active => true })
  ...
  describe '.prepare_result' do
    ...
    before do
      @current_reporting_period = create_reporting_period(@report.options[:grouping])
      @new_data = [[@current_reporting_period.previous.date_time, 1.0]]
      ReportingPeriod.stub!(:from_db_string).and_return(@current_reporting_period.previous)
      @cached = ReportCache.new
      @cached.stub!(:save!)
      ReportCache.stub!(:build_cached_data).and_return(@cached)
    end

    it 'should create :limit instances of ReportCache with value 0.0 if no new data has been read and nothing was cached' do
      ReportCache.should_receive(:build_cached_data).exactly(10).times.with(
        @report,
        @report.options[:grouping],
      ...
      ReportCache.send(:prepare_result, [], [], @report, @report.options)
    end

    it 'should create a new ReportCache with the correct value if new data has been read' do
      ReportCache.should_receive(:build_cached_data).exactly(9).times.with(
        @report,
        @report.options[:grouping],
        @report.options[:conditions],
        anything(),
      ...
      ReportCache.should_receive(:build_cached_data).once.with(
        @report,
        @report.options[:grouping],
        @report.options[:conditions],
      ...
      ReportCache.send(:prepare_result, @new_data, [], @report, @report.options)
    end

    it 'should save the created ReportCache' do
    ...
    it 'should return an array of arrays of Dates and Floats' do
      result = ReportCache.send(:prepare_result, @new_data, [], @report, @report.options)
    ...
    describe 'with :live_data = false' do
      ...
      before do
        @result = ReportCache.send(:prepare_result, @new_data, [], @report, @report.options).to_a
      end
    ...
    describe 'with :live_data = true' do
      ...
      before do
        options = @report.options.merge(:live_data => true)
        @result = ReportCache.send(:prepare_result, @new_data, [], @report, options).to_a
      end
